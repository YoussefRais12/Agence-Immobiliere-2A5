...
QSortFilterProxyModel * contrat::afficher(){
    ...
    model->setQuery("select NUMERO_CONTRAT, CIN_CLIENT, NOM_CLIENT, PRENOM_CLIENT, EMAIL_C, TELEPHONE, SIGNE, TYPE_CONTRAT, CONTENU, DATE_SIGNATURE from Contrat");
...
bool contrat::modifier(int Num)
{
    QSqlQuery query;
    QString res= QString::number(Num);
    query.prepare("update Contrat set NOM_CLIENT=:NOM_CLIENT WHERE (NUMERO_CONTRAT=:Num)");
    query.bindValue(":Num", res);
    query.bindValue(":NOM_CLIENT", Nom_client);
